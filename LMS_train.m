...
mus = [0.01, 0.05, 0.1];  % Different step sizes for LMS
    ...
    for i = 1:length(snr_levels)
        ...
        for j = 1:length(filter_lengths)
            for k = 1:length(mus)
                ...
                % Apply LMS Filter using the Clean Speech as reference
                y_ref = y_noisy - y_clean;
                [y_filtered, error_signal] = LMS_filter(y_noisy, y_ref, mu, filter_length);
